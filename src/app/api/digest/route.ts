...
<body style="margin:0;padding:0;background:#f9fafb;font-family:'Inter',Arial,sans-serif;">
  <table style="width:100%;max-width:600px;margin:0 auto;background:#ffffff;">
    ...
    <tr><td style="background:#f9fafb;padding:16px 30px;border-top:1px solid #e5e7eb;">
      <p style="margin:0;font-size:11px;color:#9ca3af;">Sandpiper Capital LLC · Industrial Outdoor Storage</p>
    </td></tr>
  </table>
</body></html>`;
}
...
export async function POST() {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: process.env.DIGEST_FROM || 'Sandpiper Capital CRM <onboarding@resend.dev>',
      to: [process.env.DIGEST_TO || 'benj.backer3@gmail.com'],
      subject: `Sandpiper Capital Morning Briefing — ${new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`,
      html,
    }),
  });
